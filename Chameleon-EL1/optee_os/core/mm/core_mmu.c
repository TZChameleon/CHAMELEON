...
#ifdef CFG_SERVICE_SLICE
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB1, VCORE_UNPG_RX_LIB1_PA,
                        VCORE_UNPG_RX_LIB1_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB2, VCORE_UNPG_RX_LIB2_PA,
                        VCORE_UNPG_RX_LIB2_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB3, VCORE_UNPG_RX_LIB3_PA,
                        VCORE_UNPG_RX_LIB3_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB4, VCORE_UNPG_RX_LIB4_PA,
                        VCORE_UNPG_RX_LIB4_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB5, VCORE_UNPG_RX_LIB5_PA,
                        VCORE_UNPG_RX_LIB5_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB6, VCORE_UNPG_RX_LIB6_PA,
                        VCORE_UNPG_RX_LIB6_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB7, VCORE_UNPG_RX_LIB7_PA,
                        VCORE_UNPG_RX_LIB7_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB8, VCORE_UNPG_RX_LIB8_PA,
                        VCORE_UNPG_RX_LIB8_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB9, VCORE_UNPG_RX_LIB9_PA,
                        VCORE_UNPG_RX_LIB9_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_LIB10, VCORE_UNPG_RX_LIB10_PA,
                        VCORE_UNPG_RX_LIB10_SZ_UNSAFE);
register_phys_mem_ul(MEM_AREA_TEE_RAM_RX_OTHERS, VCORE_UNPG_RX_OTHERS_PA,
                        VCORE_UNPG_RX_OTHERS_SZ_UNSAFE);
#endif
...
static void print_map(struct tee_mmap_region *mm __unused)
{
	DMSG("mm->pa:0x%lx, va:0x%lx, type:0x%x, size:0x%lx", mm->pa, mm->va, mm->type, mm->size);
}
...
#ifdef CFG_SERVICE_SLICE
	case MEM_AREA_TEE_RAM_RX_LIB1:
	case MEM_AREA_TEE_RAM_RX_LIB2:
	case MEM_AREA_TEE_RAM_RX_LIB3:
	case MEM_AREA_TEE_RAM_RX_LIB4:
	case MEM_AREA_TEE_RAM_RX_LIB5:
	case MEM_AREA_TEE_RAM_RX_LIB6:
	case MEM_AREA_TEE_RAM_RX_LIB7:
	case MEM_AREA_TEE_RAM_RX_LIB8:
	case MEM_AREA_TEE_RAM_RX_LIB9:
	case MEM_AREA_TEE_RAM_RX_LIB10:
	case MEM_AREA_TEE_RAM_RX_OTHERS:
#endif
		       ...
		       struct tee_mmap_region *region)
{
	...
	while (idx < end) {
#ifdef CFG_SERVICE_SLICE
		if (is_slice_tagging_enabled(tbl_info))
		{
			exceptions = thread_mask_exceptions(THREAD_EXCP_FOREIGN_INTR);
			slice_update_mapping(tbl_info, idx, pa, region->attr);
...
#ifdef CFG_SERVICE_SLICE
			if (is_slice_tagging_enabled(dir_info))
			{
				exceptions = thread_mask_exceptions(THREAD_EXCP_FOREIGN_INTR);
				slice_set_page_table_tagging(virt_to_phys(dir_info->table), dir_info->level);
				slice_set_page_table_tagging(virt_to_phys(pg_info->table), pg_info->level);
				slice_update_mapping(dir_info, idx, virt_to_phys(pg_info->table), pgt_attr);
				thread_unmask_exceptions(exceptions);
			}
			else
			{
				core_mmu_set_entry(dir_info, idx, virt_to_phys(pg_info->table), pgt_attr);
			}
#else
...
void core_mmu_map_region(struct mmu_partition *prtn, struct tee_mmap_region *mm)
{
	...
	while (size_left > 0) {
		...
		while (true) {
			...
			{
				exceptions = thread_mask_exceptions(THREAD_EXCP_FOREIGN_INTR);
				slice_update_mapping(&tbl_info, idx, paddr, mm->attr);
				thread_unmask_exceptions(exceptions);
			}
			else
			{
			      ...
			      enum teecore_memtypes memtype)
{
	...
	for (i = 0; i < num_pages; i++) {
		...
		if (is_slice_tagging_enabled(&tbl_info))
		{
			exceptions_intr = thread_mask_exceptions(THREAD_EXCP_FOREIGN_INTR);
			slice_update_mapping(&tbl_info, idx, pages[i], core_mmu_type_to_attr(memtype));
			thread_unmask_exceptions(exceptions_intr);
		}
		else
		{
			core_mmu_set_entry(&tbl_info, idx, pages[i], core_mmu_type_to_attr(memtype));
		}
#else
					 ...
					 enum teecore_memtypes memtype)
{
	...
	for (i = 0; i < num_pages; i++) {
		...
		{
			exceptions_intr = thread_mask_exceptions(THREAD_EXCP_FOREIGN_INTR);
			slice_update_mapping(&tbl_info, idx, paddr, core_mmu_type_to_attr(memtype));
			thread_unmask_exceptions(exceptions_intr);
		}
		else
		{
			core_mmu_set_entry(&tbl_info, idx, paddr, core_mmu_type_to_attr(memtype));
		}
#else
...
#ifdef CFG_SERVICE_SLICE
void slice_core_mmu_unmap_pages(vaddr_t vstart, size_t num_pages, int table_idx)
{
	struct core_mmu_table_info tbl_info;
	size_t i;
	unsigned int idx;
	...
	for (i = 0; i < num_pages; i++, vstart += SMALL_PAGE_SIZE) {
		if (!slice_core_mmu_find_table(NULL, vstart, UINT_MAX, &tbl_info, table_idx))
			panic("\033[31m[SLICE]\033[0m Can't find pagetable");

		if (tbl_info.shift != SMALL_PAGE_SHIFT)
			panic("\033[31m[SLICE]\033[0m Invalid pagetable level");
				...
				struct user_mode_ctx *uctx)
{
	...
	core_mmu_set_info_table(&pg_info, dir_info->level + 1, 0, NULL); /* cleanup ta level 3 table */

	TAILQ_FOREACH(r, &uctx->vm_info.regions, link) /* fill level 3 entries with vm_info regions */
...
#ifdef CFG_SERVICE_SLICE
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB1, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB2, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB3, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB4, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB5, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB6, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB7, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB8, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB9, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_LIB10, pa, len);
	if (!mmap)
		mmap = find_map_by_type_and_pa(MEM_AREA_TEE_RAM_RX_OTHERS, pa, len);
#endif
	/*
