			 ...
			 paddr_t pa, size_t size, uint32_t attr)
{
	...
	while (idx < end) {
#ifdef CFG_SERVICE_SLICE
		if (is_slice_tagging_enabled(ti))
		{
			slice_update_mapping(ti, idx, pa, attr);
		}
		else
		{
...
static void set_um_region(struct user_mode_ctx *uctx, struct vm_region *r)
{
	...
	if (p) {
		...
		for (ti.va_base = ROUNDDOWN(r->va, CORE_MMU_PGDIR_SIZE);
			...
			p = pgt_pop_from_cache_list(ti.va_base, uctx->ts_ctx); /* get ta page table */
			if (!p)
				continue;
			ti.table = p->tbl;
			set_reg_in_table(&ti, r);
		      ...
		      size_t align)
{
	...
	if (!mobj_is_paged(mobj)) { /* always true without CFG_PAGED_USER_TA */
		uint32_t mem_type = 0;
	...
	res = umap_add_region(&uctx->vm_info, reg, pad_begin, pad_end, align); /* select a va for mapping */
	if (res)
		goto err_put_mobj;

	res = alloc_pgt(uctx); /* check pgt table availability */
	if (res)
		goto err_rem_reg;

	if (mobj_is_paged(mobj)) { /* always false without CFG_PAGED_USER_TA */
		...
		set_um_region(uctx, reg); /* get the user ta mapping region */
	}
			...
			void *param_va[TEE_NUM_PARAMS])
{
	...
	for (n = 1, m = 0; n < TEE_NUM_PARAMS && mem[n].mobj; n++) {
		if (mem[n].mobj == mem[m].mobj && /* merge mobjs */
...
void vm_set_ctx(struct ts_ctx *ctx)
{
	...
	core_mmu_set_user_map(NULL); /* set user map entry to NULL */
	...
	if (is_user_mode_ctx(ctx)) {
		...
		core_mmu_create_user_map(uctx, &map); /* create user mapping (map->user_map) */
		core_mmu_set_user_map(&map); /* write user mapping to level 1 table entries */
		tee_pager_assign_um_tables(uctx); /* do nothing without CFG_PAGED_USER_TA  */
	}
	tsd->ctx = ctx;
}
