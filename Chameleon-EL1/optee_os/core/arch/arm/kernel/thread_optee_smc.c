			       ...
			       uint32_t a6 __unused, uint32_t a7 __maybe_unused)
{
	...
	if (a0 == OPTEE_SMC_CALL_RETURN_FROM_RPC) {
		// DMSG("a0: OPTEE_SMC_CALL_RETURN_FROM_RPC\n");
			       ...
			       struct optee_msg_arg *rpc_arg)
{
	...
	thread_rpc_shm_cache_clear(&thr->shm_cache); /* trigger OPTEE_SMC_CALL_RETURN_FROM_RPC (kernel) */
	if (rpc_arg)
		thr->rpc_arg = NULL;
