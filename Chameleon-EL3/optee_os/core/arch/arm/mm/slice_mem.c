...
const volatile uintptr_t SecureStackBase = (uintptr_t)SecureStack + sizeof(SecureStack);

/* storing data for SECURE STACK */
unsigned volatile long normal_stack[1];
unsigned volatile long normal_ttbr[1];
unsigned volatile long ptr_base_tables[1];
uint64_t volatile slice_arguments[8] SECMEM;

#if defined(COUNT_SLICE_SMC) || defined(COUNT_ALL_SMC)
int smc_counter = 0;
#endif // COUNT_SLICE_SMC || COUNT_ALL_SMC
...
/* Physical address of slice ttbr */
uint64_t phys_slice_ttbr_addr[CTX_STORAGE_SIZE][CFG_TEE_CORE_NB_CORE] SECMEM;
...
static void declare_page_table_entries(struct mmu_partition *prtn)
{
	...
	/* update physical address of slice ttbr */
	for (int i = 0; i < CTX_STORAGE_SIZE; i++)
	{
		for (size_t j = 0; j < CFG_TEE_CORE_NB_CORE; j++)
		{
			phys_slice_ttbr_addr[i][j] = virt_to_phys(slice_base_xlation_tables[i][0][j]);
		}
	}
}
...
SECURE_WRAPPER(void, slice_update_mapping, struct core_mmu_table_info *tbl_info, unsigned int idx, paddr_t pa, uint32_t attr)
{
	...
	paddr_t pa_arr[] = {table_pa, orig_pa, new_pa};
	uint64_t *tbl = tbl_info->table;
	uint64_t desc = mattr_to_desc(tbl_info->level, attr);
	uint64_t data[] = {(uint64_t)tbl, (uint64_t)idx, desc, (uint64_t)attr};
#ifdef LOG_CYCLES
	uint64_t start_cycle, end_cycle;
#endif // LOG_CYCLES
	...
	slice_update_mapping_handler((void *)secure_page_desc, pa_arr, data);
#ifdef COUNT_SLICE_SMC
	smc_counter++;
#endif // COUNT_SLICE_SMC
...
SECURE_WRAPPER(uint64_t, simple_gate, uint64_t start_cycle, uint64_t trash)
{
	uint64_t end_cycle;
...
void entry_gate_std(struct optee_msg_arg *arg)
{
	...
	// {
	// 	start_cycle = rdtsc();
	// 	end_cycle = slice_intr_handler(start_cycle, 0xdeadbeef); // trap into ATF
	// 	DMSG("SMC handler: %lu", end_cycle - start_cycle);
	// }
	// panic("SMC handler testing");

	// for (int i = 0; i < 1000; i++)
	// {
	// 	start_cycle = rdtsc();
	// 	end_cycle = simple_gate(start_cycle, 0xdeadbeef); // trap into ATF
	// 	DMSG("Secure Gate: %lu", end_cycle - start_cycle);
	// }
	// panic("Secure Gate testing");
	...
	{
#ifdef COUNT_ALL_SMC
		slice_start_counter_handler();
#endif // COUNT_ALL_SMC
		structs[0] = (uint64_t) global_slice_table_ctx;
		structs[1] = (uint64_t) slice_base_xlation_tables;
		structs[2] = (uint64_t) phys_slice_ttbr_addr;
		// get table_idx
		table_idx = slice_new_table_handler(uuid_str, structs, entry_idx);
#ifdef COUNT_SLICE_SMC
		smc_counter = 0;
		smc_counter++;
#endif // COUNT_SLICE_SMC
		...
		(0 == memcmp(uuid_str, uuid_clearkey, UUID_STR_LENGTH)))
	{
		...
		structs[0] = (uint64_t) &ptr_default_partition->base_tables;
		structs[1] = (uint64_t) global_slice_table_ctx;
		structs[2] = (uint64_t) phys_slice_ttbr_addr;
		slice_set_ttbr_handler(uuid_str, structs, entry_idx);
...
void exit_gate_std(struct optee_msg_arg *arg)
{
	...
	if (OPTEE_MSG_CMD_CLOSE_SESSION == arg->cmd) // set base_xlation_table to original
	{
		...
		slice_clear_entry_handler(uuid_str, global_slice_table_ctx, entry_idx);
#ifdef COUNT_SLICE_SMC
		smc_counter++;
		DMSG("Slice SMC called: %d", smc_counter);
#endif // COUNT_SLICE_SMC
#ifdef COUNT_ALL_SMC
		smc_counter = slice_stop_counter_handler();
		DMSG("Slice SMC called: %d", smc_counter);
#endif // COUNT_ALL_SMC
	}
