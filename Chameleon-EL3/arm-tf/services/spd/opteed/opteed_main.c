...
static int32_t opteed_init(void)
{
	 ...
	 */
	optee_entry_point = bl31_plat_get_next_image_ep_info(SECURE); /* get optee os load base address */
...
uint64_t smc_counter;
int is_counter_started;
			 ...
			 u_register_t flags)
{
	...
	else if (smc_fid == TEESMC_OPTEED_SLICE_UPDATE_MAPPING)
	{
		...
		paddr_t table_pa = pa_arr[0];
		paddr_t orig_pa = pa_arr[1];
		paddr_t new_pa = pa_arr[2];
		/* Parse data array */
		uint64_t *data = (uint64_t *) x3;
		uint64_t *tbl = (uint64_t *) data[0];
		...
		switch (page_table_update_is_valid(secure_page_desc, table_pa, orig_pa, new_pa))
		{
			case SLICE_MEMORY_INVALID:
				ERROR("Invalid mmu update!\n");
				break;
			case SLICE_MEMORY_READ_ONLY:
				ERROR("Read only memory!\n");
				break;
			case SLICE_MEMORY_AVAILABLE:
				update_secure_attr(secure_page_desc, table_pa, orig_pa, new_pa, attr);
				do_mmu_update(tbl, idx, desc, new_pa);
				break;
			default:
	...
	else if (smc_fid == TEESMC_OPTEED_SLICE_SET_TTBR)
	{
		...
		uint64_t *ptr_base_tables = (uint64_t *) structs[0];
		slice_ctx_t *global_slice_table_ctx = (slice_ctx_t *) structs[1];
		uint64_t *phys_slice_ttbr_addr __unused = (uint64_t *) structs[2];
		uint32_t entry_idx = x3;

		slice_ttbr_table_t *ptr_ttbr_table = get_slice_ttbr_table_by_uuid(global_slice_table_ctx, entry_idx, ta_uuid);
		uint64_t target_ttbr = ptr_ttbr_table->ttbr_table[linear_id];
		uint64_t target_base_table = ptr_ttbr_table->ptr_base_table;

		// write_ctx_reg(get_el1_sysregs_ctx(&optee_ctx->cpu_ctx), CTX_TTBR0_EL1, target_ttbr);
		write_ttbr0_el1(target_ttbr);
		...
		*(uint64_t *) ptr_base_tables = target_base_table;
	...
	else if (smc_fid == TEESMC_OPTEED_SLICE_NEW_TABLE)
	{
		...
		memcpy(ta_uuid, (char *) x1, UUID_STR_LENGTH);
		uint64_t *structs = (uint64_t *) x2;
		slice_ctx_t *global_slice_table_ctx = (slice_ctx_t *) structs[0];
		...
		{
			table_idx = get_ctx_table_idx(global_slice_table_ctx, ta_uuid, entry_idx, slice_base_xlation_tables, phys_slice_ttbr_addr);
		}

		SMC_RET1(&optee_ctx->cpu_ctx, table_idx);
	}
	else if (smc_fid == TEESMC_OPTEED_SLICE_CLEAR_ENTRY)
	{
		...
		if (check_uuid_bound(ta_uuid) == 0)
		{
			clear_phys_slice_table_entry(global_slice_table_ctx, entry_idx, ta_uuid);
		}
	 ...
	 */
	case TEESMC_OPTEED_RETURN_ENTRY_DONE: /* finish initializing BL31 */
		/*
		...
		uint64_t diff_cycle = x2;
		NOTICE("OPTEE initialization done in %ld cycles\n", diff_cycle);
